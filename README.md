...
#### After <br>
<hr>
처음 리액트를 공부할 때보다 훨씬 이해가 잘돼서 재미있었다. 드문드문 알고 있었던 지식이 순서대로 정리가 되는 느낌이었다.<br> 
책으로 코드를 이해하고, prettire, jsconfig.json, Scss문법, style코드 등 낯선 개념들을 검색해서 정리해 볼 수 있는 좋은 시간이었다.<br>
깃허브에는 더 정리하지 못한 부분은 블로그에 하나씩 떼어서 정리해볼 생각이다. <br>
그냥 따라치던 코드에서 '아 이래서 이렇게 되는거구나!'라고 이해가 되는 순간! 너무 기분이 좋았다. 컴포넌트 사이에 props를 내려주는 부분(?)에서 이 컴포넌트에서는 사용할 필요 없는 props를 줄줄이 내려줘야 하는 부분을 보면서 redux가 필요한 상황은 이런 상황이라는 것을 체험?하게 되었다.<br> 
